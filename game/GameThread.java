...
public class GameThread implements Runnable
{
	...
	private void guild_add_perco() 
	{
		...
		if(_perso.get_guild().getMembers().size() < Ancestra.MEMBRE_MINI_GUILDE_VALIDE)return;//Guilde invalide
	...
	private void guild_promote(String packet)
	{
		...
		Personnage p = World.getPersonnage(guid);	//Cherche le personnage a qui l'on change les droits dans la mï¿½moire
		...
		{
			int guildId = SQLManager.isPersoInGuild(guid);	//Rï¿½cupï¿½re l'id de la guilde du personnage qui n'est pas dans la mï¿½moire
	...
	private void guild_kick(String name)
	{
		...
		else
		{
			...
			{
					toRemGuild = World.getGuild(_perso.get_guild().get_id());//On prend la guilde du perso qui l'ï¿½jecte
			}
	...
	private void parseObjectPacket(String packet)
	{
		switch(packet.charAt(1))
		{
			...
			case 'D'://Depose l'objet au sol
				Object_drop(packet);
				break;
			case 'M'://Bouger un objet (Equiper/dï¿½sï¿½quiper) // Associer obvijevan
				Object_move(packet);
				break;
	...
	private synchronized void Object_move(String packet)
	{
		...
		try
		{
			...
			if ((objGUID == 9234) || (objGUID == 9233) || (objGUID == 9255) || (objGUID == 9256))
			{
				...
				SocketManager.send(_perso, "OR" + obj.getGuid()); // on le prï¿½cise au client
				...
				if ((objGUID == 9233) || (objGUID == 9234)) 
					SocketManager.GAME_SEND_ON_EQUIP_ITEM(_perso.get_curCarte(), _perso); // Si l'obvi ï¿½tait cape ou coiffe : packet au client
		...
		if(_perso.get_isTradingWith() < 0)
		{
			switch(packet.charAt(3))
			{
				...
				case '+'://Mettre un objet en vente
					...
					Objet obj = World.getObjet(itmID);//Rï¿½cupï¿½re l'item
        ...
        if (_perso.get_isTradingWith() > 0) 
        {
            ...
            if (seller != null) 
            {
                ...
                }else // si l'ï¿½change peut se faire
                {
                	...
                	Objet clone = Objet.getCloneObjet(itemStore, qua);	// on clone l'objet achetï¿½
		...
		switch(packet.charAt(2))
		{
			case '*'://Canal noir
				...
				if(msg.charAt(0) == '.')
				{
					...
					{
						if(_perso.get_fight() != null || !Ancestra.CONFIG_ALLOW_PLAYER_COMMANDS)return;
					...
					return;
                        ...
                        SocketManager.GAME_SEND_MESSAGE(_perso, "<b><a href=\""+Ancestra.CONFIG_URLVOTE+"\">Votez pour nous en cliquant sur ce lien !</a></b>", Ancestra.CONFIG_PUB_COLOR);
                        return;
                    } else if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("infos"))
                    {
                        ...
                        SocketManager.GAME_SEND_MESSAGE(_perso, ""+Ancestra.CONFIG_HELP+"", Ancestra.CONFIG_PUB_COLOR);
                        return;
                    }
				}
				if(_perso.get_fight() == null)
			...
			case ':'://Canal commerce
				...
				{
					l = (Ancestra.FLOOD_TIME  - l)/1000;//On calcul la diffï¿½rence en secondes
			...
			case '?'://Canal recrutement
				...
				{
					j = (Ancestra.FLOOD_TIME  - j)/1000;//On calcul la diffï¿½rence en secondes
			...
			case '!'://Alignement
				...
				{
					k = (Ancestra.FLOOD_TIME  - k)/1000;//On calcul la diffï¿½rence en secondes
			...
			case '^':// Canal Incarnam 
				...
				{
					x = (Ancestra.FLOOD_TIME - x)/1000;//Calculamos a diferenï¿½a em segundos 
		...
		switch(packet.charAt(1))
		{
			...
			case 'B':
	            ...
	            try
	            {
	                ...
	                SocketManager.GAME_SEND_KODE(_perso, "CK0|3");
	            }
	            catch(NumberFormatException e)
	            {
	                return;
