	        ...
	        if (map != null)
	        	if(map.get_fights() != null)
	        		if(map.getNbrFight() > 0)
	        			for (Fight f : map.get_fights().values()) {
	        				if (f == null) continue;
	        				try {
	        					f.ticMyTimer();
	        				} catch (Exception e2) {
	        					...
	        					SocketManager.GAME_SEND_cMK_PACKET_TO_ADMIN("@", 0, "DEBUG-TIC-N2", ", mapID: " + map.get_id());
	        				}
	        			}
	      	} catch (Exception e) {
	      		SocketManager.GAME_SEND_cMK_PACKET_TO_ADMIN("@", 0, "DEBUG-TIC", "ERREUR FATALE !!! Inside ticAllFightersTurns(); " + e.getMessage());
	      		SocketManager.GAME_SEND_cMK_PACKET_TO_ADMIN("@", 0, "DEBUG-TIC", ", mapID: " + map.get_id());
	      	}
	  	}
