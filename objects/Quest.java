...
//import objects.Quest.Step;
	...
	public class Quest
	{
		...
		private long _xp;
		private int _ali;
		private ArrayList<Objet.ObjTemplate> _gainob = new ArrayList<ObjTemplate>();
		private Map<Objet.ObjTemplate, Integer> _gain = new TreeMap<ObjTemplate, Integer>();
		private int _kamas;
		...
		public Quest(int ID, String step, int lvl, long xp, int ali, String gainob, String gainbb, int kamas, String name, int lastquestrequired)
		{
			...
			try
			{
				...
				while (a != ie.length) {
					a++;	
					this._step.put(Integer.valueOf(a), World.getStep(Integer.parseInt(ie[(a - 1)])));
				}
			} catch (Exception e) {
				ie = (String[])null;
			}
			try
			{
				...
				while (a != ie.length) {
					a++;
					this._gainob.add(World.getObjTemplate(Integer.parseInt(ie[(a - 1)])));
				}
			} catch (Exception e) {
				ie = (String[])null;
			}
			try
			{
				...
				while (a != ie.length) {
					...
					this._gain.put(World.getObjTemplate(Integer.parseInt(avz[0])), 
		...
		public boolean can_do(Personnage p)
		{
