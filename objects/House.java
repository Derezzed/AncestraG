...
public class House
{
			...
			if(house.getValue().get_map_id() == newMapID)
			{
				...
				if(house.getValue().get_guild_id() > 0) //Maison de guilde
				{
					...
					if(G != null)
					{
						...
						if(G.getMembers().size() < Ancestra.MEMBRE_MINI_GUILDE_VALIDE)//Ce n'est plus une maison de guilde
						{
    ...
    public void HopIn(Personnage P)
    {
        ...
        if(h == null)
            return;
        if(h.get_owner_id() == P.getAccID() || P.get_guild() != null && P.get_guild().get_id() == h.get_guild_id() && canDo(Constants.H_GNOCODE))
            OpenHouse(P, "-", true);
        else
        if(h.get_owner_id() > 0)
    ...
    public static void OpenHouse(Personnage P, String packet, boolean isHome)
    {
        if(P.get_savestat() == 0)
        {
            House h = P.getInHouse();
