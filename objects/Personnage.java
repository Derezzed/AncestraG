...
public class Personnage {
	...
	public static Personnage CREATE_PERSONNAGE(String name, int sexe, int classe, int color1, int color2, int color3,Compte compte)
	{
		...
		Personnage perso = new Personnage(
				...
				100,
				"",
				Ancestra.CONFIG_START_MAP + "," + Ancestra.CONFIG_START_CELL,
				"",
				0,
				-1,
				0,
	...
	public String parseToGM()
	{
		...
		if(_fight == null)// Hors combat
		{
			...
			if(this._guildMember!=null && this._guildMember.getGuild().getMembers().size()>=Ancestra.MEMBRE_MINI_GUILDE_VALIDE)
			{
    ...
    public String getAsPacket()
    {
        ...
        if(_fight != null)
        {
            Fight.Fighter f = _fight.getFighterByPerso(this);
            if(f != null)
            {
                pdv = f.getPDV();
                pdvMax = f.getPDVMAX();
	...
	public void levelUp(boolean send,boolean addXp)
	{
		...
		if(_lvl == 200)
			_baseStats.addOneStat(Constants.STATS_ADD_PM, 1);
		if(_lvl == 300)
			_baseStats.addOneStat(Constants.STATS_ADD_PA, 1);
		 if(_lvl == 300)
	            SocketManager.GAME_SEND_MESSAGE_TO_ALL((new StringBuilder("Bravo a (<b>")).append(get_name()).append("</b>) il vient d'atteindre le niveau <b>300</b> !").toString(), Ancestra.CONFIG_PUB_COLOR);
