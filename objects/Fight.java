...
public class Fight
{
	...
	public static class Piege
	{
		...
		public void onTraped(Fighter target)
		{
			if(target.isDead())
				return;
			_fight.get_traps().remove(this); // on enlï¿½ve le piï¿½ge sur lequel target a marchï¿½
			desappear(); //On efface le piege
			//On dï¿½clenche ses effets
			...
			cells.add(_cell);
			//on ajoute les autres cases que couvre le piï¿½ge
			for(int a = 0; a < _size;a++)
			{
				char[] dirs = {'b','d','f','h'};
				ArrayList<Case> cases2 = new ArrayList<Case>();//on ï¿½vite les modifications concurrentes
				cases2.addAll(cells); 
	...
	public static class Fighter
	{
		...
		{
			//Copie pour ï¿½vident les modif concurrentes
			...
			{
				//Si la valeur est nï¿½gative, on y touche pas
				...
				{
					//on envoie au client la desactivation de l'ï¿½tat
		...
		public void applyBeginningTurnBuff(Fight fight)
		{
			synchronized(_fightBuffs)
			{
				...
				{
					//On ï¿½vite les modifications concurrentes
		...
		public void addBuff(int id,int val,int duration,int turns,boolean debuff,int spellID,String args,Fighter caster)
		{
			...
			switch(id)
			{
				...
				case 107://Mot d'ï¿½pine (2ï¿½3), Contre(3)
				case 100://Flï¿½che Empoisonnï¿½e, Tout ou rien
		...
		public void debuff()
		{
			...
			//on vï¿½rifie chaque buff en cours, si pas dï¿½buffable, on l'ajout a la nouvelle liste
	...
	private ArrayList<Fighter> _captureur = new ArrayList<Fighter>(8);	//Crï¿½ation d'une liste de longueur 8. Les combats contiennent un max de 8 Attaquant
	...
	// Fin changement
	//TIMER dï¿½compte toutes les secondes
	...
	public void ticMyTimer()
	  {
	      if(_startTime == 0L) {// si le combat n'a pas commencï¿½
	    	  // temps qui reste ï¿½ s'ï¿½couler en ms
	          long timeRestant = (Ancestra.CONFIG_MS_FOR_START_FIGHT - 1L) - 
	          			(System.currentTimeMillis() - _ticMyTimer_startTime); 
	          if(timeRestant <= 0L) // si tout le temps est ï¿½coulï¿½
	          {
	        	  if(_type != Constants.FIGHT_TYPE_CHALLENGE)
	        		  try {// on essaye de dï¿½marrer le combat si ce n'est pas un challenge
	        			  startFight(); }
	              catch(Exception e){
	                  SocketManager.GAME_SEND_cMK_PACKET_TO_ADMIN("@", 0, "DEBUG-FIGHT", (new StringBuilder("startFight(); Dans ticMyTimer() a \351chou\351. MAPID: ")).append(get_map().get_id()).toString());
	              }
	              if(_perco != null)
	          ...
	          if(timeRestant <= 0L) // si le temps est ï¿½coulï¿½
	          {
	              _ticMyTimer_endTurn = false; // on a pas fait la fin du tour
	              _ticMyTimer_startTime = 0L; // on rï¿½initialise le dï¿½but du tour ï¿½ 0
	              try
	              {
	...
	{
		_type = type; //0: Dï¿½fie (4: Pvm) 1:PVP (5:Perco)
		_id = id;
		_map = map.getMapCopy();
		_mapOld = map;
	...
	{
		_mobGroup = group;
		_type = Constants.FIGHT_TYPE_PVM; //(0: Dï¿½fie) 4: Pvm (1:PVP) (5:Perco)
		_id = id;
		_map = map.getMapCopy();
		_mapOld = map;
		_init0 = new Fighter(this,init1);
	...
	{	
		...
		_type = Constants.FIGHT_TYPE_PVT; //(0: Dï¿½fie) (4: Pvm) (1:PVP) 5:Perco
		_id = id;
		_map = map.getMapCopy();
		_mapOld = map;
		...
		for(Personnage z : World.getGuild(_guildID).getMembers())
		{
			...
			if(z.isOnline())
			{
				...
				SocketManager.GAME_SEND_MESSAGE(z, "Un de vos percepteurs est attaque !", Ancestra.CONFIG_MOTD_COLOR);
			}
		}
	}
	...
	private void startFight()
	{
		...
		try
		{
	      if (this._type == 4)
	      {
	    	  ...
	    	  //BR,tournesol affamï¿½, Mob l'ï¿½ponge, scara dorï¿½, bworker, blops royaux, wa wab, 
	    	  ...
	    	  int challengeNumber = (inDungeon ? Ancestra.CONFIG_INDUNGEON_CHALLENGE : Ancestra.CONFIG_CHALLENGE_NUMBER);
	...
	private void startTurn()
	{
		...
		{
			if(_state >= Constants.FIGHT_STATE_FINISHED)return;
			//Si c'est ce joueur qui l'a lancï¿½
			if(g.get_caster().getGUID() == _ordreJeu.get(_curPlayer).getGUID())
			{
				//on rï¿½duit la durï¿½e restante, et si 0, on supprime
				if(g.decrementDuration() == 0)
				{
					_glyphs.remove(g);
					g.desapear();
					continue;//Continue pour pas que le joueur active le glyphe s'il ï¿½tait dessus
				}
			}
			//Si dans le glyphe
	...
	public void endTurn()
	{
		...
		try
		{
			...
			//Si empoisonnï¿½ (Crï¿½er une fonction applyEndTurnbuff si d'autres effets existent)
			for(SpellEffect SE : _ordreJeu.get(_curPlayer).getBuffsByEffectID(131))
			{
				...
				if(_ordreJeu.get(_curPlayer).hasBuff(184))
				{
					...
					dgt = dgt-_ordreJeu.get(_curPlayer).getBuff(184).getValue();//Rï¿½duction physique
				}
	...
	public boolean fighterDeplace(Fighter f, GameAction GA)
	{
		...
		{
			for(Fighter T : tacle)//Les stabilisï¿½s ne taclent pas
			{ 
				if(T.isState(6)) 
				{ 
					tacle.remove(T); 
			...
			if(!tacle.isEmpty())//Si tous les tacleur ne sont pas stabilisï¿½s
			{
				...
				if(rand > chance)
				{
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7,GA._id, "104",_ordreJeu.get(_curPlayer).getGUID()+";", "");//Joueur taclï¿½
        ...
        else//Si le joueur est planquï¿½ x)
        {
        	if(_ordreJeu.get(_curPlayer).getPersonnage() != null)
        	{
        ...
        && po.isState(Constants.ETAT_PORTEUR))
        {
        	...
        	//si le joueur va bouger
       		if(nextCellID != po.get_fightCell().getID())
       		{
       			...
       			//La nouvelle case sera dï¿½finie plus tard dans le code
        ...
        if(po != null) po.get_fightCell().addFighter(po);// mï¿½me erreur que tantï¿½t, bug ou plus de fighter sur la case
       if(nStep < 0) 
       {
        ...
        {
        	try {
    			Thread.sleep(900+100*nStep);//Estimation de la durï¿½e du dï¿½placement
    		} catch (InterruptedException e) {};
	...
	public int tryCastSpell(Fighter fighter,SortStats Spell, int caseID)
	{
		...
		if(CanCastSpell(fighter,Spell,Cell, -1))
		{
			...
			SocketManager.GAME_SEND_GAS_PACKET_TO_FIGHT(this, 7, fighter.getGUID()); // infos concernant la dï¿½pense de PA ?
			...
			// le client ne peut continuer sans l'envoi de ce packet qui annonce le coï¿½t en PA
		...
		if(fighter.getPersonnage() != null)
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 102,fighter.getGUID()+"",fighter.getGUID()+",-0"); // annonce le coï¿½t en PA
		try {
			Thread.sleep(500);
		} catch (InterruptedException e) {};
		_curAction = "";
	...
	public boolean CanCastSpell(Fighter fighter, SortStats spell, Case cell, int launchCase)
	{
		...
		// Pour peur si la personne poussï¿½e a la ligne de vue vers la case
		char dir = Pathfinding.getDirBetweenTwoCase(ValidlaunchCase, cell.getID(), _map, true);
		if(spell.getSpellID() == 67)
			if(!Pathfinding.checkLoS(_map, Pathfinding.GetCaseIDFromDirrection(ValidlaunchCase, dir, _map, true), cell.getID(), null)) {
				if(Ancestra.CONFIG_DEBUG) GameServer.addToLog("("+_curPlayer+") Le sort demande une ligne de vue, mais la case visee n'est pas visible pour le joueur");
				if(perso != null)
					SocketManager.GAME_SEND_Im_PACKET(perso, "1174");
				return false;
	...
	public String GetGE(int win)
    {
        ...
        // si c'est un combat PVM alors bonus potentiel en ï¿½toiles
       ...
       // int TEAM2lvl = 0;
        ...
        //Evaluation de la prï¿½sence de la traque
        for(Fighter F : TEAM2)
        {
        	if(F.isInvocation())continue;
      ...
      //Challenge augmente la PP totale (atteint plus facilement les seuils)
		...
		if ((this._type == 4) && (this._challenges.size() > 0)) {
  	     ...
  	     factChalDrop += _mobGroup.get_bonusValue(); // on ajoute le bonus en ï¿½toiles
		}
 		factChalDrop /= 100;
	        ...
	        for(Fighter F : TEAM2)
	        {
	        	//Evaluation de l'argent ï¿½ gagner
	        	...
	        	for(Drop D : F.getMob().getDrops())
	        	{
	        		...
	        		{
	        			int taux = (int)(D.get_taux()*Ancestra.CONFIG_DROP);
	        ...
	        //On Rï¿½ordonne la liste des combattants en fonction de la PP
	        ...
	        while(Temp.size() < TEAM1.size())
	        {
		        ...
		        for(Fighter F : TEAM1)
		        {
	        		//S'il a plus de PP et qu'il n'est pas listï¿½
	    ...
	    //XP SYSTEM
	        ...
	        if ((this._type == 4) && (this._challenges.size() > 0)) {
     	       try
     	       {
     	    	   ...
     	    	   totalGainXp += _mobGroup.get_bonusValue(); // on ajoute le bonus en ï¿½toiles
		...
		//Capture d'ï¿½mes
	        ...
	        if(isCapturable)
	        {
		        ...
		        for(Fighter F : TEAM2)	//Crï¿½ation de la pierre et verifie si le groupe peut ï¿½tre capturï¿½
		        {
		        	if(!isFirst)
		        		pierreStats += "|";
		        	
		        	pierreStats += F.getMob().getTemplate().getID() + "," + F.get_lvl();//Converti l'ID du monstre en Hex et l'ajoute au stats de la futur pierre d'ï¿½me
		        ...
		        pierrePleine = new PierreAme(World.getNewItemGuid(),1,7010,Constants.ITEM_POS_NO_EQUIPED,pierreStats);	//Crï¿½e la pierre d'ï¿½me
		        
		        for(Fighter F : TEAM1)	//Rï¿½cupï¿½re les captureur
		        {
		        ...
		        if(_captureur.size() > 0 && !World.isArenaMap(get_map().get_id()))	//S'il y a des captureurs
	    		{
	    			for (int i = 0; i < _captureur.size(); i++)
	    			{
	    				try
	    				{
			        		Fighter f = _captureur.get(Formulas.getRandomValue(0, _captureur.size()-1));	//Rï¿½cupï¿½re un captureur au hasard dans la liste
			        		if(!(f.getPersonnage().getObjetByPos(Constants.ITEM_POS_ARME).getTemplate().getType() == Constants.ITEM_TYPE_PIERRE_AME))
		    				{
			    				_captureur.remove(f);
		    					continue;
		    				}
			    			Couple<Integer,Integer> pierreJoueur = Formulas.decompPierreAme(f.getPersonnage().getObjetByPos(Constants.ITEM_POS_ARME));//Rï¿½cupï¿½re les stats de la pierre ï¿½quippï¿½
	    ...
	    for(Fighter i : TEAM1)
		{
        	...
        	if(type == Constants.FIGHT_TYPE_CHALLENGE)
        	{
    			...
    			int PP = i.getTotalStats().getEffect(Constants.STATS_ADD_PROS);
    			...
    			while(!allIsDropped) {
    				...
    				allIsDropped = _type != Constants.FIGHT_TYPE_PVT;
    				if(possibleDrops.isEmpty())
						allIsDropped = true;
    			}
    	        if(i._id == captWinner && pierrePleine != null)	//S'il ï¿½ capturï¿½ le groupe
        		{
	...
	public void verifIfTeamAllDead()
	{
		...
		if(team0 || team1 || !verifyStillInFight())
		{
			...
			switch(_type)//Team mobs sauf en dï¿½fi/aggro
			{
			...
			for(Fighter F : winTeam)
			{
				if(F._Perco != null)
				{
					...
					for(Personnage z : World.getGuild(_guildID).getMembers())
					{
						...
						{
							SocketManager.GAME_SEND_gITM_PACKET(z, Percepteur.parsetoGuild(z.get_guild().get_id()));
							SocketManager.GAME_SEND_MESSAGE(z, "Votre percepteur remporte la victoire.", Ancestra.CONFIG_MOTD_COLOR);
						}
					}
	...
	public void onFighterDie(Fighter target, Fighter caster) 
	{
		...
		if(!target.hasLeft()) deadList.put(target.getGUID(), target);//on ajoute le joueur ï¿½ la liste des cadavres ;)
		SocketManager.GAME_SEND_FIGHT_PLAYER_DIE_TO_FIGHT(this,7,target.getGUID());
		target.get_fightCell().getFighters().clear();// Supprime tout causait bug si portï¿½/porteur
		...
		{ 
			...
			f.setState(Constants.ETAT_PORTE, 0);//J'ajoute ceci quand mï¿½me pour signaler qu'ils ne sont plus en ï¿½tat portï¿½/porteur
		...
		if(target.getTeam() == 0)
		{
			...
			for(Entry<Integer,Fighter> entry : team.entrySet())
			{
				...
				if(entry.getValue().getInvocator().getGUID() == target.getGUID())//si il a ï¿½tï¿½ invoquï¿½ par le joueur mort
				{
	 ...
	 public boolean CanCastSpell3(Fighter fighter, Sort.SortStats spell, Carte.Case cell)
	  {
	    ...
	    if ((dist < spell.getMinPO()) || (dist > MaxPO))
	    {
	      if (Ancestra.CONFIG_DEBUG) GameServer.addToLog("La case est trop proche ou trop eloignee(1) Min: " + spell.getMinPO() + " Max: " + spell.getMaxPO() + " Dist: " + dist);
	      if (perso != null)
	      {
	        SocketManager.GAME_SEND_Im_PACKET(perso, "1171;" + spell.getMinPO() + "~" + spell.getMaxPO() + "~" + dist);
	      }
	  ...
	  public boolean CanCastSpell2(Fighter fighter, Sort.SortStats spell, Carte.Case cell, int launchCase)
	  {
	    ...
	    if ((spell.hasLDV()) && (!Pathfinding.checkLoS(_map, launchCase, cell.getID(), fighter)))// Si problŽme de cellID
	    {
	      if (Ancestra.CONFIG_DEBUG) GameServer.addToLog("Le sort demande une ligne de vue, mais la case visee n'est pas visible pour le joueur (IA)");
	      return false;
	    }
	    ...
	    {
	      if (Ancestra.CONFIG_DEBUG) GameServer.addToLog("La case est trop proche ou trop eloignee(2) (IA) Min: " + spell.getMinPO() + " Max: " + spell.getMaxPO() + " Dist: " + dist);
	      return false;
	    }
	    if (!LaunchedSort.coolDownGood(fighter, spell.getSpellID())) {
	      return false;
	...
	public void tryCaC(Personnage perso, int cellID)
	{
		...
		if(_ordreJeu.get(_curPlayer).getGUID() != caster.getGUID())//Si ce n'est pas a lui de jouer
			return;
		 // Pour les challenges, vï¿½rif sur CaC
		if ((this._type == 4) && (this._challenges.size() > 0) && !this._ordreJeu.get(this._curPlayer).isInvocation() && !this._ordreJeu.get(this._curPlayer).isDouble() && !this._ordreJeu.get(this._curPlayer).isPerco())
		{
			for (Entry<Integer, Challenge> c : this._challenges.entrySet()) {
				if (c.getValue() == null) 
		...
		if(perso.getObjetByPos(Constants.ITEM_POS_ARME) == null)//S'il n'a pas de CaC
		{
			...
			if(target != null)
			{
				...
				finalDommage = SpellEffect.applyOnHitBuffs(finalDommage,target,caster,this);//S'il y a des buffs spï¿½ciaux
	...
	public void leftFight(Personnage perso, Personnage target)
	{
		...
		if(F != null)
		{
			...
			switch(_type)
			{
				case Constants.FIGHT_TYPE_CHALLENGE://Dï¿½fie
				...
				case Constants.FIGHT_TYPE_PVT://Perco
					if(_state >= Constants.FIGHT_STATE_ACTIVE)
					{
						...
						if(!StillInFight)//S'arrï¿½te ici si il ne reste plus personne dans le combat et dans la team
						{
							//Met fin au combat
							verifIfTeamAllDead();
						}else
						{
							...
							//si c'ï¿½tait a son tour de jouer
							if(_ordreJeu.get(_curPlayer) == null)return;
							if(_ordreJeu.get(_curPlayer).getGUID() == F.getGUID())
							{
								endTurn();
					...
					}else if(_state == Constants.FIGHT_STATE_PLACE)
					{
						...
						if(isValid1)//Celui qui fait l'action a lancer le combat et leave un autre personnage
						{
							...
							{
								if(Ancestra.CONFIG_DEBUG) System.out.println("EXLUSION DE : "+T.getPersonnage().get_name());
						...
						}else if(T == null)//Il leave de son plein grï¿½ donc (T = null)
						{
							...
							if(isValid2)//Soit il a lancer le combat => annulation du combat
							{
								...
								_state = 4;//Nous assure de ne pas dï¿½marrer le combat
								World.getCarte(_map.get_id()).removeFight(_id);
								SocketManager.GAME_SEND_MAP_FIGHT_COUNT_TO_MAP(World.getCarte(_map.get_id()));
	...
	public void joinAsSpect(Personnage p)
	{
		...
		long timeRestant = (Ancestra.CONFIG_MS_PER_TURN - 1) - (System.currentTimeMillis() - _ticMyTimer_startTime);
		p.get_curCell().removePlayer(p.get_GUID());
		SocketManager.GAME_SEND_GJK_PACKET(p, _state, 0, 0, 1, timeRestant, _type);
